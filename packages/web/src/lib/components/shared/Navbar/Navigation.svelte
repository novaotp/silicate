...
<div class="md:hidden block text-neutral-950">
    <nav class="relative w-full flex justify-between px-5 py-[10px] h-[60px]">
        ...
        <button
            class="relative flex flex-col h-full min-w-10 items-center justify-evenly gap-1 p-2 text-center rounded-md
                    bg-red-500 duration-150 ease-linear hover:bg-red-600 text-sm z-[999]"
        ...
        <Notifications bind:show={showNotifications} bind:notificationButtonNode />
    </nav>
    {#if showMenu}
...
<nav class="hidden md:flex flex-col justify-between items-center h-full {showMenu ? "w-[200px]" : "w-20"} duration-300 overflow-x-hidden ease-in-out p-5 bg-neutral-100">
    ...
    <ul class="relative w-full flex flex-col gap-3">
        <li class="relative flex w-full items-center justify-center">
            <button
                class="relative flex flex-col h-full w-full min-w-10 items-center justify-evenly gap-1 p-2 text-center rounded-md
                        bg-red-500 duration-150 ease-linear hover:bg-red-600 text-sm z-[999]"
                on:click={() => (showNotifications = true)}
                bind:this={notificationButtonNode}
            >
