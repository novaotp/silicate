...
<FullScreen.Backdrop class="flex-center" on:click={closeModal}>
    <Card class="{confirmCardClasses} md:max-w-2xl">
        <form method="post" action="?/createExam" use:enhance={createEnhance} class="flex flex-col gap-5">
            ...
            <div class="w-full flex justify-end items-center gap-5">
                ...
                <Button.Normal type="submit" disabled={title === '' || !score || !weight}>Créer</Button.Normal>
            </div>
        </form>
    </Card>
</FullScreen.Backdrop>
