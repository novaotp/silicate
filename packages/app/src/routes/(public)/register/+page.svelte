<script lang="ts">
	...
	const onFormSubmit = async (): Promise<void> => {
		...
		try {
			...
			resetPasswords();
			console.error(
				`Something went wrong whilst registering a new user : ${(error as Error).message}`
			);
...
<form
	...
	class="relative mb-5 flex w-full flex-col overflow-x-hidden xl:hidden"
>
	<Switcher>
		...
		<svelte:fragment slot="credentials">
			...
			<div class="flex flex-col gap-[10px]">
				...
				{#if password.length < 8}
					<span class="text-accent-danger-500 text-sm">
                        Minimum 8 caractères attendus.
                    </span>
				{/if}
			</div>
			<div class="flex flex-col gap-[10px]">
				...
				{:else if password !== confirmPassword}
					<span class="text-accent-danger-500 text-sm">Les mots de passe ne correspondent pas.</span>
				{/if}
			</div>
		</svelte:fragment>
	</Switcher>
...
<form
	...
	class="relative mb-5 w-[70%] flex-col justify-center items-center gap-5 px-0.5 overflow-x-hidden hidden xl:flex"
>
	...
	<div class="mt-5">
		<Button.Normal type="submit">Créer mon compte</Button.Normal>
	</div>
</form>
