...
export const updateGradebook = async ({ gradebookId, name, description, from, to }: UpdateGradebookProps): Promise<boolean> => {
  try {
    const userId = await useServerUserId();
    ...
    const now = new Date();
    const query = 'UPDATE public.gradebook SET name = $1, description = $2, start_period = $3, end_period = $4, updated_at = $5 WHERE user_id = $6 AND id = $7;';
...
export const deleteGradebook = async (gradebookId: string): Promise<boolean> => {
  try {
    const userId = await useServerUserId();
    ...
    const query = 'DELETE FROM public.gradebook WHERE id = $1 AND user_id = $2;';
