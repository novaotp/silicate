...
const AuthLink = ({ href, label, variant }: AuthLinkProps) => {
    const colors =
        variant === "accent"
            ? "bg-primary-color text-opposite-text-color"
            : "bg-bg-color text-primary-color border border-primary-color";

    return (
        <Link
            className={`relative my-5 w-full rounded-lg bg-bg-color p-3 text-center ${colors}`}
            href={href}
        >
            {label}
        </Link>
