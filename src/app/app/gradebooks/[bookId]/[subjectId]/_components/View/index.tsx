...
export const View = ({ grades }: ViewProps): JSX.Element => {
  ...
  const sortedAndFilteredGrades = useMemo(() => {
    ...
    return grades
      .filter(grades => searchQuery === "" || grades.name.toLowerCase().includes(searchQuery.toLowerCase()))
      .sort((grade1, grade2) => grade2.updated_at.getTime() - grade1.updated_at.getTime());
  }, [grades, searchQuery]);
    ...
    <ul className={styles.notes} ref={ref}>
      {
        !grades
          ...
          : sortedAndFilteredGrades.length === 0
            ...
            : sortedAndFilteredGrades.map((grade: Grade) => <GradeCard key={grade.id} grade={grade} />)
      }
    </ul>
  );
}
