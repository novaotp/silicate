...
export const View = ({ memos }: ViewProps): JSX.Element => {
  ...
  const sortMemos = useCallback((memos: Memo[]): Memo[] => {
    return memos.sort((memo1, memo2) => memo2.updated_at.getTime() - memo1.updated_at.getTime());
  }, [memos]);
  ...
  return (
    <ul className={styles.notes}>
      {
        !memos
        ? <Loading text="Chargement des mémos..." />
        : memos.length === 0
